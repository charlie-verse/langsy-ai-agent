...
export const send = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check current message count for the user in this chat
    const userMessages = await ctx.db
      .query("messages")
      .withIndex("by_chat_role", (q) =>
        q.eq("chatId", args.chatId).eq("role", "user")
      )
      .collect();

    if (userMessages.length >= 10) {
      throw new Error("Message limit reached for this chat [10 messages].");
    }
